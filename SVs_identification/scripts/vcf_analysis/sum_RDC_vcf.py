...
plt.ylabel('# SVs')
plt.savefig('RDC_count_per_genome.png')
...
plt.xticks(fontsize=4, rotation=45, ha='right')
plt.ylabel('# SVs')
plt.savefig('RDC_filt_count_per_genome.png')
...
top_svs = {k: v for (k, v) in count_filt.items() if v > 5}
...
merged_all_svs.to_csv('RDC_all_svs.csv', sep=',')
