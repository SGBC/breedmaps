...
options <- list(
  ...
  make_option(c("-e", "--evaDir"), help = "EVA gff file directory from ENSEMBL", default =
                "data/eva/remapped/"),
...
dataset_path = paste(params$workingDir,
                     params$evaDir,
                     sep = "")
dataset_file_names = list.files(path = dataset_path, pattern = "*.gff")
dataset = as.data.frame(rtracklayer::import(paste(dataset_path,dataset_file_names[[1]],sep="")))
...
for (i in 1:(length(gene_file_names))) {
  ...
  filt_results = paste(
    params$workingDir,params$resultsDir,
    "dataset/",
    "dataset_",
    gene_file_names[[i]],
    sep = ""
