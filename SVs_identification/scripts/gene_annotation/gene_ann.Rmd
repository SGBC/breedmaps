...
for (i in 1:length(vcf_files)) {
  ...
  filt_var_final = filt_var %>% dplyr::mutate(SV_LENGTH = as.numeric(END)-as.numeric(POS)) %>% dplyr::rename(ID1 = ID)
...
for (i in 1:length(range_list)) {
  # overlap_gene = findoverlap_dataframe(range_list[[i]], gene_range, df_list[[i]], gene_df) %>% dplyr::rename(ID = ID1, gene_id = ID2)
  overlap_tran = findoverlap_dataframe(range_list[[i]], tran_range, df_list[[i]], tran_df) %>% dplyr::rename(ID = ID1, gene_id = ID2)
  # overlap_exon = findoverlap_dataframe(range_list[[i]], exon_range, df_list[[i]], exon_df) %>% dplyr::rename(ID = ID1, gene_id = ID2)
  # overlap_rest = findoverlap_dataframe(range_list[[i]], rest_range, df_list[[i]], rest_df) %>% dplyr::rename(ID = ID1, gene_id = ID2)
