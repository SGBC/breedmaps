...
This is a power heavy script so it needs to be run in parts. It is divided into: annotation of genes, exon, transcripts and the rest.
...
# gene_df = ann_df %>% filter(type == "gene") %>% dplyr::rename(ID2 = gene_id)
# tran_df = ann_df %>% filter(type == "transcript") %>% dplyr::rename(ID2 = gene_id)
# exon_df = ann_df %>%filter(type=="exon") %>% dplyr::rename(ID2 = gene_id)
# rest_df = ann_df %>% filter(type != "exon") %>% filter( type != "gene") %>% filter(type != "transcript") %>% dplyr::rename(ID2 = gene_id)
