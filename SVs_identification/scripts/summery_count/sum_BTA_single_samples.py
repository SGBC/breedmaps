...
vcf_files_raw = glob.glob(home_path + "/breedmaps/SVs_identification/data/vcf/BTA*")
vcf_files_filt = glob.glob(home_path + "/breedmaps/SVs_identification/results/filtered_variants/precise_BTA*")
...
top_svs = {pos: count for (pos, count) in count_filt.items() if count > 25}
